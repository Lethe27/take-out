...
public class SearchHttpAKUtil {
    ...
    public String  getLocation(String address) throws Exception {
        SearchHttpAKUtil snCal = new SearchHttpAKUtil();

        Map params = new LinkedHashMap<String, String>();
        params.put("address",address);
        params.put("output", mapProperties.getOutput());
        params.put("ak", mapProperties.getAk());
        params.put("callback", "showLocation");


        return snCal.requestGetAK(mapProperties.getUrl(), params);
    }
    /**
     * 默认ak
     * 选择了ak，使用IP白名单校验：
